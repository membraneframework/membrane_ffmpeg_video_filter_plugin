...
UNIFEX_TERM filter(UnifexEnv *env, UnifexPayload *payload, State *state) {
  ...
  if (av_buffersrc_add_frame_flags(state->buffersrc_ctx, frame,
    ...
    res = filter_result_error(env, "error_feeding_filtergraph");
    goto exit_filter;
  }
  ...
  if (ret < 0) {
    res = filter_result_error(env, "error_pulling_from_filtergraph");
    goto exit_filter;
  }
  ...
  if (av_image_copy_to_buffer(payload_frame.data, payload_size,
                              ...
                              1) < 0) {
    res = filter_result_error(env, "copy_to_payload");
    goto exit_filter;
  }
  res = filter_result_ok(env, &payload_frame);
exit_filter:
  av_frame_free(&frame);
...
void create_filter_description(char *filter_descr, int len, char *text,
  ...
  if (strcmp(x, "center") == 0) {
    ...
    filter_descr += snprintf(filter_descr, len, ":x=%s", x);
  }
  if (strcmp(y, "center") == 0) {
    ...
    filter_descr += snprintf(filter_descr, len, ":y=%s", y);
  }
}
